...
// `financing` is optional — when omitted (or empty), the zone covers the full cost. When
// given, the financiers together cover the summed `share` (0-1) and the zone covers the remainder.
export async function createTrust(placementId: string, resource: string, financing: Financing[] = []): Promise<void> {
  return apiPost('/api/trusts', { placementId, resource, payment: financing })
}

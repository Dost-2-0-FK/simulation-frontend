...
// Design coordinate space all marker math below is computed in (matches the showcase).
const DESIGN_SIZE = 54
// Bitmap resolution registered with MapLibre, for crisp rendering when scaled/zoomed.
const RASTER_SIZE = 128
// Tells MapLibre "this RASTER_SIZE bitmap represents a DESIGN_SIZE-css-px icon at icon-size:1".
...
function iconGlyph(key: string, sz: number, color: string): string {
  const path = ICON_PATHS[key]
  if (!path) return ''
  const scale = (sz * 0.58) / 512
  const offset = sz * 0.21
  return `<g transform="translate(${offset.toFixed(1)},${offset.toFixed(1)}) scale(${scale.toFixed(4)})"><path fill="${color}" d="${path}"/></g>`
}

function markerSvg(spec: IconSpec): string {
  const { shape, key, bloc, state } = spec
  const c = state === 'disabled' ? PALETTE.disabled : PALETTE[bloc]
  ...
  if (shape === 'empty') {
    ...
    if (key === 'crosshair') {
      ...
      inner = `<line x1="${h}" y1="16" x2="${h}" y2="${sz - 16}" stroke="${c.bg}" stroke-width="2.5" stroke-linecap="round"/><line x1="16" y1="${h}" x2="${sz - 16}" y2="${h}" stroke="${c.bg}" stroke-width="2.5" stroke-linecap="round"/>`
    } else {
      inner = `<circle cx="${h}" cy="${h}" r="4.5" fill="${c.bg}"/>`
    }
  } else if (shape === 'base') {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${sz} ${sz}" width="${RASTER_SIZE}" height="${RASTER_SIZE}"${opacity}>${ring}${body}${inner}</svg>`
}
...
// can look up what to render without parsing the id string back apart.
const specsById = new Map<string, IconSpec>()

export function registerIconSpec(spec: IconSpec): string {
  const id = iconId(spec)
  specsById.set(id, spec)
  return id
}

export function getIconSpec(id: string): IconSpec | undefined {
  return specsById.get(id)
}

export function loadIconImage(spec: IconSpec): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    const img = new Image()
    img.onload = () => resolve(img)
    img.onerror = () => reject(new Error(`Failed to load placement icon for ${iconId(spec)}`))
    img.src = `data:image/svg+xml;base64,${btoa(markerSvg(spec))}`
  })
}

// Derives the icon to show for a placement. bloc/state/sub-type aren't modeled on
// Placement yet (no zone/bloc ownership for empty placements, no resource type for
// trusts) — defaults below are placeholders until those fields exist on the backend model.
export function specForPlacement(placement: Placement): IconSpec {
  if (!placement.occupant) {

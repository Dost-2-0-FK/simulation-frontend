...
import { ICON_PIXEL_RATIO, UNIT_ICON_SIZE_FACTOR, getIconSpec, loadIconImage, registerIconSpec, specForUnit } from './placementIcons'
...
function toGeoJSON(units: Unit[]): FeatureCollection<Point, { id: string; icon: string }> {
  return {
    type: 'FeatureCollection',
    features: units.map((u) => ({
      type: 'Feature',
      geometry: { type: 'Point', coordinates: [u.lng, u.lat] },
      properties: { id: u.id, icon: registerIconSpec(specForUnit(u)) },
    })),
  }
}

export default function UnitsLayer({ map, units, onUnitHover }: Props) {
  ...
  // when the hovered feature actually changes.
  ...
  useEffect(() => {
    ...
    const ensureLayer = () => {
      ...
      map.addSource(SOURCE_ID, {
        type: 'geojson',
        data: toGeoJSON(unitsRef.current),
      })
      ...
      map.addLayer({
        ...
        layout: {
          'icon-image': ['get', 'icon'],
          'icon-size': UNIT_ICON_SIZE_FACTOR,
  ...
  // Push data updates into the existing source whenever units change.
  useEffect(() => {
    if (!map) return
    const source = map.getSource(SOURCE_ID) as maplibregl.GeoJSONSource | undefined
    source?.setData(toGeoJSON(units))
  }, [map, units])
